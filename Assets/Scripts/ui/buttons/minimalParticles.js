...
// Minimal Particles toggle button
...
var state : boolean = false; //If minimal particles are enabled on or not
...
function Action () {
	...
	// Toggle Minimal Particles
	if (state == true){
		gameObject.renderer.material.mainTexture = off; // reset the texture
		PlayerPrefs.SetInt("minimalParticles",0); // Turn off minimal particles
	}
	else{
